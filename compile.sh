...
echo
echo "$blue***********************************************"
echo "          Compiling Oinc.Bugx™.anDroid Kernel...          "
echo "***********************************************$nocol"
echo ""
#woods defconfig
...
echo "$yellow Copying to outdir/OincBugxanDroid $nocol"
cp outdir/arch/arm/boot/zImage-dtb outdir/OincBugxanDroid/Image
...
█     █  █ █▄  ▄▀ █  █  ▐█ █ █  █ █   █ 
 █       █ ▀███▀    █    ▐ █  █ █  ███  
  ▀     █          ▀       █   ██       
       ▀  "
echo  "$cyan***********************************************"
echo "          Packing Oinc.Bugx™.anDroid Kernel...          "
echo  "***********************************************$nocol"
echo ""
echo  "$yellow Putting Oinc.Bugx™.anDroid Kernel in Recovery Flashable Zip $nocol"
#using lazy kernel flasher
cd outdir
cd OincBugxanDroid
    if 
    [ -f outdir/OincBugxanDroid/out_done ] 
    then
    rm -rf out_done
    else
    make
    ...
    echo ""
    echo ""
    echo "" "Locate Oinc.Bugx™.anDroid Kernel in the following path : "
    echo "" "outdir/OincBugxanDroid/out_done"
    echo ""
echo  "$green▄   █ ▄▄  █    ████▄ ██   ██▄   ▄█    ▄     ▄▀  
   █  █   █ █    █   █ █ █  █  █  ██     █  ▄▀    
█   █ █▀▀▀  █    █   █ █▄▄█ █   █ ██ ██   █ █ ▀▄  
...
█▄ ▄█  █        ▀         █ ███▀   ▐ █  █ █  ███  
 ▀▀▀    ▀                █           █   ██       
    ...
    echo
    echo  "$blue***********************************************"
    echo "      Uploading Oinc.Bugx™.anDroid Kernel to Web[https://transfer.sh/]"
    echo  "***********************************************$nocol"
    echo ""
    echo " l.o.a.d.i.n.g..."
    sleep 0.4;
    ...
    sleep 0.1;
    echo ""
    curl --upload-file outdir/OincBugxanDroid/out_done/OincBugxanDroid-Ngt-woods*.zip https://transfer.sh/OincBugxanDroid-Ngt-woods_Nougat_$BUILD_START.zip
    echo ""
    echo ""
    echo " uPLOADING dONE !!!"
    echo ""
